...
export const DeliveryModal = ({ isOpen, onClose, accounts, onSuccess }: Props) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (method === 'transfer' && !accountId) {
      alert("Selecciona la cuenta donde recibiste el dinero");
      return;
    }
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          ...
          <div className="grid grid-cols-2 gap-3">
            <button 
              ...
              className={`flex flex-col items-center gap-2 p-4 rounded-2xl border-2 transition-all ${method === 'cash' ? 'border-primary bg-primary/5 text-primary' : 'border-border text-text-secondary'}`}
            >
              <Banknote size={24} />
            ...
            <button 
              ...
              className={`flex flex-col items-center gap-2 p-4 rounded-2xl border-2 transition-all ${method === 'transfer' ? 'border-primary bg-primary/5 text-primary' : 'border-border text-text-secondary'}`}
            >
              <CreditCard size={24} />
          ...
          {/* Selector de Cuenta (Solo si es transferencia) */}
          {method === 'transfer' && (
          ...
          >
            {loading ? <Loader2 className="animate-spin" size={24} /> : <><Save size={20} /> Guardar Registro</>}
          </button>
        </form>
      </div>
    </div>
